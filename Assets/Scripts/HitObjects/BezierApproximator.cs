...
namespace OsuPlayground.HitObjects
{
    public class BezierApproximator
    {
        ...
        private static bool IsFlatEnough(Vector2[] controlPoints)
        {
            ...
            {
                if ((controlPoints[i - 1] - 2 * controlPoints[i] + controlPoints[i + 1]).sqrMagnitude > tolerance_sq * 4)
                {
                    return false;
                }
            }
        ...
        {
            Vector2[] midpoints = this.subdivisionBuffer1;
            ...
            {
                l[i] = midpoints[0];
                r[this.count - i - 1] = midpoints[this.count - i - 1];
        ...
        {
            Vector2[] l = this.subdivisionBuffer2;
            Vector2[] r = this.subdivisionBuffer1;

            Subdivide(controlPoints, l, r);

            for (int i = 0; i < this.count - 1; ++i)
            {
                l[this.count + i] = r[i + 1];
            }
